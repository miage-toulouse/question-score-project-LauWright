...
public class QuestionAChoixMultiple implements QuestionAChoix {
     ...
     */
    public QuestionAChoixMultiple(String enonce, List<Integer> indicesBonnesReponses, int nbpropositions) {
        this.enonce = enonce;
    ...
    public float getScoreForIndice(int indiceChoix) {
        int nbBonne = indicesBonnesReponses.size();
        int mauvaise = this.nbpropositions - nbBonne;
        if (indicesBonnesReponses.contains(indiceChoix)) {
            return 100f / ((float) (indicesBonnesReponses.size()));
        } else {
            return -100f / ((float) (mauvaise));
        }
    }
}
